...
public final class ChatOverlayRenderer {
    ...
    public void render(GuiGraphicsExtractor context, boolean preview, boolean interactiveMode) {
        linkHits.clear();
        messageHits.clear();
        widgetHits.clear();
    ...
    private int renderMessage(GuiGraphicsExtractor context, Minecraft client, ChatMessage message,
    ...
    private int drawBadges(GuiGraphicsExtractor context, Minecraft client, List<ChatMessage.Badge> badges, int x, int y, int badgeHeight, String fontId) {
        int cursor = x;
        for (ChatMessage.Badge badge : badges) {
            ...
            if (icon.isPresent()) {
                ...
                context.blit(RenderPipelines.GUI_TEXTURED, tex.id(), cursor, y, 0f, 0f, badgeHeight, badgeHeight, tex.width(), tex.height(), tex.width(), tex.height());
            ...
            context.text(client.font, styled(style.abbreviation(), fontId), cursor + 3, y + (badgeHeight - 8) / 2, 0xFFFFFFFF, true);
            cursor += chipWidth + 3;
        }
        return cursor;
    }
    ...
    private int drawLine(GuiGraphicsExtractor context, Minecraft client, ChatLayoutEngine.LaidOutLine line,
        ...
        for (ChatLayoutEngine.PlacedToken placed : line.tokens()) {
            ...
            if (placed.token() instanceof MessageToken.Emote emote) {
                ...
                if (tex.isPresent()) {
                    ...
                    context.blit(RenderPipelines.GUI_TEXTURED, t.id(), emoteX, emoteY, 0f, 0f, drawW, drawH, t.width(), t.height(), t.width(), t.height());
                }
                ...
                context.text(client.font, styled(text.text(), fontId), drawX, drawY, color, true);
                if (text.link()) {
    ...
    private void drawLines(GuiGraphicsExtractor context, Minecraft client, List<ChatLayoutEngine.LaidOutLine> lines,
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            for (ChatLayoutEngine.PlacedToken placed : lines.get(i).tokens()) {
                if (placed.token() instanceof MessageToken.Text text) {
                    context.text(client.font, styled(text.text(), fontId), baseX + placed.x(), y, color, true);
                }
            }
        }
    }
