...
public final class TwitchHudChatScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
        renderer.render(graphics, false, true);
        rebuildCopyButtons();

        Optional<ChatOverlayRenderer.MessageHit> hovered = renderer.messageAt(mouseX, mouseY);
        hovered.ifPresent(hit -> graphics.fill(hit.x(), hit.y(), hit.x() + hit.width(), hit.y() + hit.height(), 0x22FFFFFF));
        ...
        for (ChatOverlayRenderer.LinkHit button : copyButtons) {
            ...
            graphics.drawString(this.font, "C", button.x() + (button.width() - textWidth) / 2, button.y() + 1, 0xFFFFFFFF, false);
        }

        Optional<ChatOverlayRenderer.LinkHit> hoveredButton = copyButtonAt(mouseX, mouseY);
        if (hoveredButton.isPresent()) {
            graphics.drawString(this.font, "Click: copy link", mouseX + 12, mouseY, 0xFFFFFFFF, true);
        } else if (hovered.isPresent()) {
            graphics.drawString(this.font, "Click: copy - Right-click: hide user", mouseX + 12, mouseY, 0xFFAAAAAA, true);
        }
