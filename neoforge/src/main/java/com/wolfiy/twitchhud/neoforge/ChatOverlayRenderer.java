...
public final class ChatOverlayRenderer {
    ...
    private static Component styled(String text, String fontId) {
        return "off".equals(fontId) ? Component.literal(text) : Component.literal(text).setStyle(PlatformImpl.styleFor(fontId));
    }
    ...
    public void render(GuiGraphics graphics, boolean preview) {
        render(graphics, preview, false);
    }

    public void render(GuiGraphics graphics, boolean preview, boolean interactiveMode) {
        linkHits.clear();
        messageHits.clear();
        widgetHits.clear();
        ...
        graphics.fill(x, y, x + width, y + height, bgAlpha << 24);
        ...
        for (int i = messages.size() - 1; i >= 0 && cursorY > y; i--) {
            ...
            int blockTop = renderMessage(graphics, client, message, x, y, width, cursorY, lineHeight, messageColor, linkColor, config);
        ...
        renderWidgets(graphics, client, config, preview);
    }

    private int renderMessage(GuiGraphics graphics, Minecraft client, ChatMessage message,
        ...
        if (message.kind() != ChatMessage.Kind.CHAT) {
            ...
            graphics.fill(x, blockTop, x + width, cursorY, 0x552ECC71);
            ...
            drawLines(graphics, client, layout.lines(), drawX, drawY,
        ...
        if (isMentioned(message, config.channel())) {
            graphics.fill(x, blockTop, x + width, cursorY, 0x33FFD24D);
        }
        ...
        if (!badges.isEmpty() && blockTop >= y) {
            ...
            firstLineX = drawBadges(graphics, client, badges,
        ...
        for (int lineIdx = 0; lineIdx < layout.lines().size(); lineIdx++) {
            ...
            charOffset = drawLine(graphics, client, layout.lines().get(lineIdx),
    ...
    private int drawBadges(GuiGraphics graphics, Minecraft client, List<ChatMessage.Badge> badges, int x, int y, int badgeHeight, String fontId) {
        int cursor = x;
        for (ChatMessage.Badge badge : badges) {
            ...
            if (icon.isPresent()) {
                ...
                graphics.blit(tex.id(), cursor, y, badgeHeight, badgeHeight, 0f, 0f, tex.width(), tex.height(), tex.width(), tex.height());
                cursor += badgeHeight + 3;
                continue;
            }

            BadgeStyle.Style style = BadgeStyle.forSetId(badge.setId());
            int textWidth = client.font.width(style.abbreviation());
            int chipWidth = textWidth + 6;
            graphics.fill(cursor, y, cursor + chipWidth, y + badgeHeight, 0xFF000000 | style.color());
            graphics.drawString(client.font, styled(style.abbreviation(), fontId), cursor + 3, y + (badgeHeight - 8) / 2, 0xFFFFFFFF, false);
            cursor += chipWidth + 3;
        }
        return cursor;
    }

    private int badgeWidth(Minecraft client, ChatMessage.Badge badge, int badgeHeight) {
        if (badgeTextures.get(badge.setId(), badge.version()).isPresent()) {
            return badgeHeight + 3;
        }
        BadgeStyle.Style style = BadgeStyle.forSetId(badge.setId());
        return client.font.width(style.abbreviation()) + 9;
    }
    ...
    private int drawLine(GuiGraphics graphics, Minecraft client, ChatLayoutEngine.LaidOutLine line,
        ...
        for (ChatLayoutEngine.PlacedToken placed : line.tokens()) {
            ...
            if (placed.token() instanceof MessageToken.Emote emote) {
                ...
                if (tex.isPresent()) {
                    ...
                    graphics.blit(t.id(), emoteX, emoteY, drawW, drawH, 0f, 0f, t.width(), t.height(), t.width(), t.height());
                }
                ...
                graphics.drawString(client.font, styled(text.text(), fontId), drawX, drawY, color, true);
                if (text.link()) {
    ...
    private void drawLines(GuiGraphics graphics, Minecraft client, List<ChatLayoutEngine.LaidOutLine> lines,
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            for (ChatLayoutEngine.PlacedToken placed : lines.get(i).tokens()) {
                if (placed.token() instanceof MessageToken.Text text) {
                    graphics.drawString(client.font, styled(text.text(), fontId), baseX + placed.x(), y, color, true);
                }
            }
        }
    }
