...
public final class TwitchHudConfigScreen extends Screen {
    ...
    private EditBox widgetColorField;
    private EditBox newWordField;
    ...
    protected TwitchHudConfigScreen(
        ...
        super(Component.literal("TwitchHUD Settings"));
        this.parent = parent;
    ...
    private void initChat(
        ...
        colorField.setResponder(this::onColorFieldChanged);
    ...
    private void initWidgets(
        ...
        addWidgetRow(left, y, contentW, "Viewers",
                config.showViewerWidget(),
                () -> config.showViewerWidget(
                        !config.showViewerWidget()),
                "viewer");
        y += 22;
        addWidgetRow(left, y, contentW, "Title",
                config.showTitleWidget(),
                () -> config.showTitleWidget(
                        !config.showTitleWidget()),
                "title");
        y += 22;
        addWidgetRow(left, y, contentW, "Hype Train",
                config.showHypeTrainWidget(),
                () -> config.showHypeTrainWidget(
                        !config.showHypeTrainWidget()),
                "hype");
        y += 22;
addWidgetRow(left, y, contentW, "Followers",
                config.showFollowerWidget(),
                () -> config.showFollowerWidget(
                        !config.showFollowerWidget()),
                "follower");
        y += 22;
        addWidgetRow(left, y, contentW, "Creator goals",
    ...
    private void addWidgetRow(
        ...
        int toggleWidth = contentW - settingsWidth - 4;
        buttons.add(new RoundedButton(
                left, y, toggleWidth, 20,
                label + " " + onOff(enabled),
                () -> {
                    toggle.run();
                    persist();
    ...
    private void initWidgetStyle(
        ...
        buttons.add(new RoundedButton(
                left + 32, y, contentW - 64, 20,
                widgetDisplayName(selectedWidgetKey),
                () -> {
                }
        ));
        buttons.add(new RoundedButton(
                left + contentW - 28, y, 28, 20, ">",
                () -> cycleWidgetStyle(1)
        ));
        y += 24;
        ...
        widgetColorField = new EditBox(
                font,
                left + 4,
                y + 2,
                64,
                18,
                Component.literal("Widget color")
        );
        widgetColorField.setBordered(false);
        widgetColorField.setMaxLength(6);
        widgetColorField.setValue(
                String.format("%06X", style.textColor())
        );
        widgetColorField.setResponder(
                this::onWidgetColorFieldChanged
        );
        addRenderableWidget(widgetColorField);
    ...
    private void cycleWidgetStyle(int direction) {
        List<String> keys = widgetStyleKeys();
        int index = Math.max(0, keys.indexOf(selectedWidgetKey));
        selectedWidgetKey = keys.get(
                Math.floorMod(index + direction, keys.size())
        );
        rebuild();
    }
    ...
    private void onWidgetColorFieldChanged(String value) {
        ...
        config.widgetStyle(selectedWidgetKey)
                .textColor(Integer.parseInt(text, 16));
        markDirty();
    }
    ...
    private void initFilters(
        ...
        newWordField = new EditBox(
                font,
                left + 4,
                y + 2,
                contentW - 88,
                18,
                Component.literal("Blocked word")
        );
        newWordField.setBordered(false);
        newWordField.setMaxLength(32);
        addRenderableWidget(newWordField);
        buttons.add(new RoundedButton(
                left + contentW - 78,
                y,
                78,
                ...
                () -> {
                    config.addBlockedWord(
                            newWordField.getValue()
                    );
                    newWordField.setValue("");
                    persist();
                    rebuild();
                }
        ));
    ...
    private void initProfiles(
        ...
        addRenderableWidget(profileField);
        y += 26;
    ...
    private void exportSettings() {
        if (profileStore.exportConfig(config)) {
            ...
            actionStatus = "Exported - path copied";
        } else {
    ...
    public void render(
        ...
        renderer.render(graphics, true, true);
