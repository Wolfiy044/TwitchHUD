...
public final class ChatOverlayRenderer {
    ...
    public void render(DrawContext context, boolean preview) {
        render(context, preview, false);
    }
    ...
    private int drawBadges(DrawContext context, MinecraftClient client, List<ChatMessage.Badge> badges, int x, int y, int badgeHeight, String fontId) {
        ...
        for (ChatMessage.Badge badge : badges) {
            ...
            if (icon.isPresent()) {
                ...
                context.drawTexture(tex.id(), cursor, y, badgeHeight, badgeHeight, 0f, 0f, tex.width(), tex.height(), tex.width(), tex.height());
        ...
        for (ChatLayoutEngine.PlacedToken placed : line.tokens()) {
            ...
            if (placed.token() instanceof MessageToken.Emote emote) {
                ...
                if (tex.isPresent()) {
                    ...
                    context.drawTexture(t.id(), emoteX, emoteY, drawW, drawH, 0f, 0f, t.width(), t.height(), t.width(), t.height());
                }
