...
public final class RoundedSlider {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        graphics.fill(x, y, x + fillWidth, y + height, FILL_COLOR);
        int handleX = x + fillWidth;
        graphics.fill(Math.max(x, handleX - 1), y - 1, Math.min(x + width, handleX + 2), y + height + 1, HANDLE_COLOR);

        String label = labelFormatter.apply(value);
        var font = Minecraft.getInstance().font;
        int textWidth = font.width(label);
        graphics.drawString(font, label, x + width / 2 - textWidth / 2, y + (height - 8) / 2, 0xFFFFFFFF, true);
    }
